...
conn:connect(80, Url:match("(.*%w+%.%w+)%/.*")) --parse domain from Url
if ReqType == "POST" then
conn:send(PostRequest(node.chipid(),Url:match("[%w]*%..*%w+%.%w+%/(.*)"),arg[1]))
print ("Json Parsed POST arg[1]: "..jsonparser(arg[1]))
else
conn:send(GetRequest(node.chipid(),Url:match("[%w]*%..*%w+%.%w+%/(.*)"),arg[1]))
print ("Json Parsed POST arg[1]: "..urlparser(arg[1]))
end